...
public static void main(String... args){
	...
	System.out.println("index ans"+test[binarysearch(test, 0, test.length-1, 5)]);*/
	...
	while(true){
		...
		while(nextpostiontobecoverd<=N){
			int mynextpot =  findpot(allowed,searchfromthisindex,nextpostiontobecoverd,potrange);
			if(mynextpot==-1){
				...
				nextpostiontobecoverd = allowed[mynextpot]+potrange+1;
			}
			
		}
		if(isSolutionExists && (sprinklr==null||cost(templist.size(),potrange)<cost(noofsprinklr,range))){
		sprinklr =templist;
		noofsprinklr =sprinklr.size();
		range = potrange;
		}/*else if(isSolutionExists) break; */// break while loop we have the optimum solutuion.
		if(isSolutionExists &&( templist.size()<=1 /*|| (sprinklr!=null && templist.size()>=sprinklr.size())*/))break;
	}
		////////////////////////////////////////////////////
	System.out.println(noofsprinklr +" "+range);
	for(int a:sprinklr){
		System.out.print(a+" ");
	}
	System.out.println();
...
private static int findpot(int[] allowed2, int searchfromthisindex,
	...
	// do a binary search on an given array starting form the search index given.
	...
	if(ans<start||ans>end){
		// ans is not found for this perticular range sarch for the next range.
		return -1;
	}
	return ans;
	
}
private static int binarysearch(int[] allowed2, int start, int end, int search) {
	if(start==end){
		if (allowed2[start]==search) return start;
	...
	if(allowed2[middle]==search)return middle;
	else if(allowed2[middle]>search) end = middle -1;
	else{
		start = middle +1;
	}
